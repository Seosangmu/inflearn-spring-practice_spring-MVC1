...
public class FrontControllerServletV4 extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Map<String, String> paramMap = createParamMap(request); // paramMap
        Map<String, Object> model = new HashMap<>(); // model추가

        String viewName = controllerV4.process(paramMap, model);

        WebView view = viewResolver(viewName);
        view.render(model, request, response);
    }

    private static WebView viewResolver(String viewName) {
        return new WebView("/WEB-INF/views/" + viewName + ".jsp");
    }
